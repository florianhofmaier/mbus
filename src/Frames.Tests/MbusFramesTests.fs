...
let parseFrame buf =
    Frame.parse { Off = 0; Buf = ReadOnlyMemory<uint8>(buf) }
...
let ``parse Cnf frame when byte is 0xE5 should return Cnf`` () =
    let buf = [| 0xE5uy; |]
    parseValidFrame buf |> should equal Cnf
...
let ``parse Short frame when frame is valid SND-NKE should return short frame`` () =
    ...
    let expected = Frame.Short { PrmAdr = 0x01uy; CField = 0x40uy }
